...
} from './validation/ai-output.validation';
...
export class AiArticlesService {
  ...
  async summarize(articleId: string, dto: SummarizeArticleDto) {
    ...
    if (cached) {
      this.aiUsageService.recordCacheHit();
      return cached;
    }
    this.aiUsageService.recordCacheMiss();
  ...
  async translate(articleId: string, dto: TranslateArticleDto) {
    ...
    const validated = validateTranslateOutput(
      parsed,
      raw,
  ...
  async analyze(
    ...
    const raw = await this.geminiService.generate(prompt);
    const parsed = this.parseJson(raw);
    const validated = validateAnalyzeOutput(parsed, raw);
